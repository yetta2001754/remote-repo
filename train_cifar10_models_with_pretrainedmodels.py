...
cifar_testset = torchvision.datasets.CIFAR10(root='./data', train=False, download=False, transform=transform_test)
...
def train(epoch):
    ...
    for batch_idx, (inputs, targets) in enumerate(trainloader):
        ...
        #在这里添加新的loss函数
        loss = criterion(outputs, targets) #原始的计算loss值

        loss.backward(retain_graph=True) #反向传播，计算每一层的梯度值
        loss2 = torch.norm(inputs.grad, p=2)#计算模型更新前关于x的梯度的值
...
def test(epoch):
    ...
    if acc > best_acc:
        ...
        if not os.path.isdir('checkpoint-ln-pre'):
            os.mkdir('checkpoint-ln-pre')
        torch.save(state, './checkpoint-ln-pre/cifar10-ckpt-%s-%dto%d-dp%.3f-droplayer%.3f-seed%d.t7' % (args.net,
                                                                                                     args.eidx,
                                                                                                     args.sidx,
                                                                                                     args.train_dp,
                                                                                                     args.droplayer,
    ...
    torch.save(state, './checkpoint-ln-pre/cifar10-ckpt-%s-%dto%d-dp%.3f-droplayer%.3f-seed%d-latest.t7' % (args.net,
                                                                                                        args.eidx,
                                                                                                        args.sidx,
                                                                                                        args.train_dp,
                                                                                                        args.droplayer,
